...
final_preds = FALSE # NB - check which prior round final_preds is looking for
...
final_preds_on_test_set_reporting_by_timeslice = TRUE
...
scores_file <- paste0("EDcrowding/predict-admission/data-output/xgb_",model_features, "_", model_period, "_scores_single_",today(),".rda")
...
imps_file <- paste0("EDcrowding/predict-admission/data-output/xgb_",model_features, "_", model_period, "_imps_single_",today(),".rda")
...
#timeslices <- c("000")
timeslices <- c("_single")

if (base_model | tune_nr | tune_trees | tune_samples | final_preds) {
  ...
  for (ts_ in timeslices) {
    ...
    # load timeslice 
    if (ts_ == "_single") {
      inFile = paste0("EDcrowding/predict-admission/data-raw/dm", ts_, "_", dm_single_file_date, ".rda")
    } else {
...
# Final preds on test set reporting by timeslice -------------------------------------------------



if (final_preds_on_test_set_reporting_by_timeslice) {
  ...
  # load dm_single design matrix once 
  inFile = paste0("EDcrowding/predict-admission/data-raw/dm", "_single", "_", dm_single_file_date, ".rda")
  load(inFile)
  
  name_ts <- paste0("dm", "_single")
  
  
  # get model output file date - not dissertation
  name_ = paste0("model_output_date_", model_period, "_single")
  model_output_file_date = get(name_)
  
  
  # get file to add any missing features
  features_file <- paste0("EDcrowding/predict-admission/data-output/xgb_",model_features,
                          "_", gsub(" +", "", model_period),
                          "_features_","task_single","_",model_output_file_date, ".rda")
  load(features_file)
  ...
  learner_file <- paste0("EDcrowding/predict-admission/data-output/xgb_",model_features,
                         "_", gsub(" +", "", model_period),
                         "_learner_","task_single","_",model_output_file_date, ".rda")
  ...
  for (ts_ in timeslices) {
    ...
    # reduce dt to only those patients left at the timeslice
    dt = dt[elapsed >= as.numeric(ts_)]
    
    
    # arrival window already in single model
    # dt = merge(dt, summ[, .(csn, a_arrival_window)], by = "csn")
    ...
    # train on full training set and save results on validation set
    scores <- save_results_test_set(name_tsk, tsk = NA, learner, tsk_train_ids, tsk_val_ids, tsk_ids = "test",
                                    tuning_round = "final_preds", scores, model_features, model_period,
                                    labels = ts$adm == 1)
