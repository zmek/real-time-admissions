      ...
      for (time_window_ in time_window_array_) {
        ...
        if (!(i == length(time_pts) |
          ...
          for (h in 1 : nrow(df)) {
            ...
            base_prob$surv_ = base_prob$surv ^ in_ED_all[csn == df$csn[h] & time_pt == time_pts[i], epow]
            ...
            tta_prob_ = bind_rows(tta_prob_, data.table(csn = df$csn[h], 
                                                        ...
                                                        # timeslice = base_prob$surv_$timeslice[j],
                                                        # prob_still_in_now,
...
# load file with number in each location
load(paste0("EDcrowding/flow-mapping/data-raw/num-in-ED-at-arrival-time_",num_in_ED_at_arrival_time_file_date,".rda"))

if (use_test_set_for_Cox_SDEC) {
  
  # load Cox regression results (survival probs and regression coefs)
  load(paste0("EDcrowding/real-time/model-input/cox_surv_SDEC_test_set_",cox_surv_date_SDEC_test,".rda"))
  load(paste0("EDcrowding/real-time/model-input/cox_results_SDEC_test_set_",cox_surv_date_SDEC_test, ".rda"))
  load(paste0("EDcrowding/real-time/model-input/scale_params_SDEC_test_set_",cox_surv_date_SDEC_test, ".rda"))
...
if (tsk_ids %in% c("val", "test")) {
  ...
  # for (model_period_ in c("Pre + Post", "Post-SDEC")) {
    ...
    in_ED_all = get_cox_coef_by_csn(in_ED_all = adm_preds[[1]], coefs, FALSE, num_in_ED, model_period_, scale_params_, use_test_set_for_Cox_SDEC)
    ...
    if (use_test_set_for_Cox_SDEC & model_period_ == "Post-SDEC" & tsk_ids == "test") {
      
      prob_dist_file = paste0("EDcrowding/predict-admission/model-output/model_eval_xgb_", model_features, "_", model_period_, "_",
                              tsk_ids, "_with_test_set_for_Cox_", Sys.Date(),".rda")
    }
  ...
  for (model_period_ in c("Pre", "Post", "Pre + Post", "Post-SDEC")) {
    ...
    start_of_set = period_dates[1]
    end_of_set = period_dates[2]
    ...
    time_pts = sample(time_pts, round(length(time_pts)/10), replace = FALSE, prob = NULL)
    
    # get time to admit trained on training data
    poisson_results = poisson_results[model_period == model_period_]
    poisson_nya = poisson_not_yet_arrived_all[model_period == model_period_]
    
    poisson_means = get_poisson_means(time_pts, poisson_results, poisson_nya)
    ...
    in_ED_all = get_cox_coef_by_csn(in_ED_all = adm_preds[[1]], coefs, FALSE, num_in_ED, model_period_, scale_params_)
