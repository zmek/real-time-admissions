...
Before running any of the below, the scripts in the folder called flow-mapping need to be run, and summ_file_date parameter updated in config.R
...
Note that the cox equation's coefficients are used by the script  generate-prob-dist-and-pt-estimates.R to make predictions in a function called get_cox_coef_by_csn() [Note this function has moved to utils file]
...
## Seven step pipeline: Order to run files in
...
Before running, set the required dates for training, validation and test sets as paramters in config.R
...
# In these cases don't update config.R with today's date
...
Runs XGBoost on a single model (no timeslices)
...
Prints charts showing outputs from tuning

### reporting on class balance
